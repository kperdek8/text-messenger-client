defmodule TextMessengerClientWeb.HomePage do
  ...
  def mount(_params, session, socket) do
    ...
    if token == nil do
      ...
      {:ok, assign(socket, websocket: websocket, messages: messages, chats: chats, selected_chat: chat_id, users: users, token: token)}
    end
  end
	...
	<div id="main" class="w-screen h-screen flex bg-black gap-4 p-2">
	  ...
	  <div id="chat_entries" class="w-1/5 h-full bg-gray-900 p-2 border-gray-700 rounded-lg">
		<div id="chat_list" class="flex flex-col w-full h-full overflow-y-auto bg-gray-900 p-2 rounded-lg">
		  ...
		  <!-- Create New Chat Button (Below Chat List) -->
			<button class="mt-4 p-2 bg-green-500 hover:bg-green-600 text-white rounded-lg w-full">
			Create New Chat
			</button>
		</div>
	  </div>
	  ...
	  <div id="user_list" class="w-1/6 flex flex-col h-full border-gray-700">
		<div id="chat_members" class="flex flex-col h-full overflow-y-auto bg-gray-900 p-2 rounded-lg">
		  ...
		  <button class="mt-4 p-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg w-full">
			Add User to Chat
		  </button>
		</div>
	  </div>

	  <!-- Logged In Information (Bottom Left) -->
	  <div id="logged-in-info" class="absolute bottom-4 left-4 text-white">
		<span>Logged in as: DO IMPLEMENTACJI</span>
		<button class="ml-2 p-2 bg-red-500 hover:bg-red-600 text-white rounded-lg">
		  Logout
		</button>
	  </div>
	</div>
